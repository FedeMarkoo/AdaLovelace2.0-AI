 ...
 */
public class Matematica extends Objeto {

	public static String calcular(String mensaje) {
	...
	public static double calcula(String cad) {
		while (cad.contains("(")) {
			...
			cad = cad.substring(0, ini - 1) + calcula(subcad) + restocad;
		}

		if (cad.contains("%")) {
			int div = cad.indexOf("%");
			if (div != 0)
				return (calcula(cad.substring(0, div)) * calcula(cad.substring(div + 1)) / 100);
			return calcula(cad.substring(div + 1));
		}

		if (cad.contains("+")) {
			int div = cad.indexOf("+");
			if (div != 0)
				return calcula(cad.substring(0, div)) + calcula(cad.substring(div + 1));
			return calcula(cad.substring(div + 1));
		}
		...
		if (cad.contains("^")) {
			int div = cad.indexOf("^");
			if (div != 0)
				return Math.pow(calcula(cad.substring(0, div)), calcula(cad.substring(div + 1)));
			return 0;
		}
